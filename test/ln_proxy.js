...
test('destination already exists', async t => {
  try {
    await shell.ln(`${t.context.tmp}/file1`, `${t.context.tmp}/file2`);
  } catch (e) {
    t.truthy(shell.error());
    t.is(e.code, 1);
  }
...
test('non-existent source', async t => {
  try {
    await shell.ln(`${t.context.tmp}/noexist`, `${t.context.tmp}/linkfile1`);
  } catch (e) {
    t.truthy(shell.error());
    t.is(e.code, 1);
  }
...
test('non-existent source (-sf)', async t => {
  try {
    await shell.ln('-sf', 'no/exist', `${t.context.tmp}/badlink`);
  } catch (e) {
    t.truthy(shell.error());
    t.is(e.code, 1);
  }
...
test('non-existent source (-f)', async t => {
  try {
    await shell.ln('-f', 'noexist', `${t.context.tmp}/badlink`);
  } catch (e) {
    t.truthy(shell.error());
    t.is(e.code, 1);
  }
...
test('basic usage', async t => {
  const result = await shell.ln(`${t.context.tmp}/file1`, `${t.context.tmp}/linkfile1`);
...
test('With glob', async t => {
  try {
    await shell.rm(`${t.context.tmp}/linkfile1`);
  } catch (e) {
    // nothing
  }
  const result = await shell.ln(`${t.context.tmp}/fi*1`, `${t.context.tmp}/linkfile1`);
  t.truthy(fs.existsSync(`${t.context.tmp}/linkfile1`));
  t.is(
    fs.readFileSync(`${t.context.tmp}/file1`).toString(),
    fs.readFileSync(`${t.context.tmp}/linkfile1`).toString()
...
test('-s option', async t => {
  utils.skipOnWinForEPERM(await shell.ln.bind(shell, '-s', 'file2', `${t.context.tmp}/linkfile2`), () => {
...
test('Symbolic link directory test', async t => {
  await shell.mkdir(`${t.context.tmp}/ln`);
  await shell.touch(`${t.context.tmp}/ln/hello`);
  const result = await shell.ln('-s', 'ln', `${t.context.tmp}/dir1`);
  t.truthy(fs.existsSync(`${t.context.tmp}/ln/hello`));
  t.truthy(fs.existsSync(`${t.context.tmp}/dir1/hello`));
  t.is(result.code, 0);
});

test('To current directory', async t => {
  await shell.cd(t.context.tmp);
  let result = await shell.ln('-s', './', 'dest');
  t.is(result.code, 0);
  await shell.touch('testfile.txt');
  t.truthy(fs.existsSync('testfile.txt'));
  t.truthy(fs.existsSync('dest/testfile.txt'));
  await shell.rm('-f', 'dest');
  await shell.mkdir('dir1');
  await shell.cd('dir1');
  result = await shell.ln('-s', './', '../dest');
  t.is(result.code, 0);
  await shell.touch('insideDir.txt');
  await shell.cd('..');
  t.truthy(fs.existsSync('testfile.txt'));
  t.truthy(fs.existsSync('dest/testfile.txt'));
...
test('-f option', async t => {
  const result = await shell.ln('-f', `${t.context.tmp}/file1.js`, `${t.context.tmp}/file2.js`);
  t.is(result.code, 0);
...
test('-sf option', async t => {
  utils.skipOnWinForEPERM(await shell.ln.bind(shell, '-sf', 'file1.txt', `${t.context.tmp}/file2.txt`), () => {
...
test('Abspath regression', async t => {
  utils.skipOnWinForEPERM(await shell.ln.bind(shell, '-sf', 'file1', path.resolve(`${t.context.tmp}/abspath`)), () => {
...
test('Relative regression', async t => {
  utils.skipOnWinForEPERM(await shell.ln.bind(shell, '-sf', 'file1.txt', `${t.context.tmp}/file2.txt`), () => {
