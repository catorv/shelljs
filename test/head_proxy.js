...
test('file does not exist', async t => {
  ...
  try {
    await shell.head('/asdfasdf'); // file does not exist
  } catch (e) {
    t.truthy(shell.error());
    t.is(e.code, 1);
    t.is(e.stderr, 'head: no such file or directory: /asdfasdf');
  }
});
...
test('directory', async t => {
  ...
  try {
    await shell.head('test/resources/');
  } catch (e) {
    t.truthy(shell.error());
    t.is(e.code, 1);
    t.is(e.stderr, "head: error reading 'test/resources/': Is a directory");
  }
});
...
test('simple', async t => {
  const result = await shell.head('test/resources/head/file1.txt');
  t.falsy(shell.error());
  t.is(result.code, 0);
...
test('multiple files', async t => {
  const result = await shell.head('test/resources/head/file2.txt',
...
test('reading more lines than are in the file (no trailing newline)', async t => {
  const result = await shell.head('test/resources/file2', 'test/resources/file1');
  t.falsy(shell.error());
  t.is(result.code, 0);
...
test('reading more lines than are in the file (with trailing newline)', async t => {
  const result = await shell.head('test/resources/head/shortfile2',
...
test('Globbed file', async t => {
  const result = await shell.head('test/resources/head/file?.txt');
  t.falsy(shell.error());
  t.is(result.code, 0);
...
test('With `\'-n\' <num>` option', async t => {
  const result = await shell.head('-n', 4, 'test/resources/head/file2.txt',
    'test/resources/head/file1.txt');
  t.falsy(shell.error());
  t.is(result.code, 0);
  t.is(result.toString(), topOfFile2
...
test('negative values (-num) are the same as (numLines - num)', async t => {
  const result = await shell.head('-n', -46, 'test/resources/head/file1.txt');
  t.falsy(shell.error());
  t.is(result.code, 0);
  t.is(result.toString(), 'file1 1\nfile1 2\nfile1 3\nfile1 4\n');
});

test('right-hand side of a pipe', async t => {
  const result = (await shell.cat('test/resources/head/file1.txt')).head();
  t.falsy(shell.error());
  t.is(result.code, 0);
  t.is(result.toString(), topOfFile1.slice(0, 10).join('\n') + '\n');
});
